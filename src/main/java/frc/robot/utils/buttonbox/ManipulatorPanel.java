...
public class ManipulatorPanel extends ButtonBoxPanel {
  ...
  private static final TROUGH_ID = 0;
  private static final LEFT_POLE_ID = 4;
  private static final EJECT_ID = 6;
  ...
  public Trigger trough() { //level 1 coral
    return new Trigger(() -> getButton(TROUGH_ID) && !getButton(TROUGH_ID + 1)); //only activate if algaeLow won't activate
  }
  ...
  public Trigger leftPole() {
    return new Trigger(() -> getButton(LEFT_POLE_ID) && !getButton(LEFT_POLE_ID + 1)); //only activate if not both are pressed
  }
  ...
  public Trigger processor() {
    //TODO: check whether the robot has an algae or not to determine whether to do processor or intake
