...
public class Vision extends PeriodicRunnable {
  ...
  public Pose2d getPose() {
    return Pose3d.kZero.plus(lastSee.minus(zero)).toPose2d();
  }
  ...
  public void periodic() {
    ...
    diff = slamInputs.frames - lastFrame;
    ...
    if (slamInputs.present) {
      ...
      Logger.recordOutput("Oculus_rotation", rotation);
      var cpos =
          new Pose3d(
      ...
      //          Pose3d.kZero.plus(
      //              new Transform3d(
      //                  (Pose3d.kZero.plus(slamPose)).plus(new Transform3d(cpos, Pose3d.kZero)),
      //                  Pose3d.kZero));
      cpos = Pose3d.kZero.plus(slamPose.inverse());
      ...
      if (new XboxController(0).getXButtonPressed()) {
        setpos(new Pose2d(5, 3, new Rotation2d()));
      }
    }
